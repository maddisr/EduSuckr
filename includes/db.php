<?php
    ...
    class DB {
        ...
        function setEduCourse($param) {
            $query = "INSERT into ".DB_PREFIX."educourses (course_guid, title, description, posts, comments, course_tag, course_blog, course_wiki, signup_deadline, course_starting_date, course_ending_date) values (".$param['course_guid'].",'".$param['title']."','".$param['description']."','".$param['posts']."','".$param['comments']."','".$param['course_tag']."','".$param['course_blog']."','".$param['course_wiki']."','".$param['signup_deadline']."','".$param['course_starting_date']."','".$param['course_ending_date']."') ON DUPLICATE KEY UPDATE title='".$param['title']."', description='".$param['description']."', posts='".$param['posts']."', comments='".$param['comments']."', course_tag='".$param['course_tag']."', course_blog='".$param['course_blog']."', course_wiki='".$param['course_wiki']."', signup_deadline='".$param['signup_deadline']."', course_starting_date='".$param['course_starting_date']."', course_ending_date='".$param['course_ending_date']."'";
        ...
        function getCourseData($param, $type="post") {
            ...
            $post_id = "";
            if ($type=="comment") $post_id = " ,post_id";
            $query = "SELECT DISTINCT id, c.link as link, title, content, date, author, blogger_id, base".$post_id." FROM ".DB_PREFIX.$type."s c LEFT JOIN ".DB_PREFIX."course_rels_".$type."s r ON c.link=r.link WHERE r.course_guid=".$param[0]." ORDER BY date DESC LIMIT ".$limit;
            $result = $this->query($query);
            $comments = array();
            if(mysql_num_rows($result)) {
        ...
        function getCommentsByPost($post) {
            $comments = array();
            if ($post) {
                $query = "SELECT id, link, title, content, date, author, blogger_id, base, post_id FROM ".DB_PREFIX."comments WHERE post_id=".$post." ORDER BY date ASC";
                $result = $this->query($query);
                while($comment = mysql_fetch_array($result)) {
                    if ($comment['blogger_id']) {
                        if ($fn = $this->getFullnameByBloggerId($comment['blogger_id'])) {
