<?php
    ...
    class Suckr {
        ...
        function writeComment($title, $link, $base, $date, $content, $author_name, $blogger_id) {
            ...
            $query = "INSERT into ".DB_PREFIX."comments (link, base, title, date, content, author, blogger_id, post_id, post_author) values ('".$link."', '".$base."', '".$title."', '".$date."', '".$content."', '".$author_name."', '".$blogger_id."', '".$pre_res['id']."', '".$pre_res['author']."') ON DUPLICATE KEY UPDATE title='".$title."', date='".$date."', content='".$content."', author='".$author_name."', blogger_id='".$blogger_id."', post_id='".$pre_res['id']."', post_author='".$pre_res['author']."', modified=NOW()";
            return $db->query($query);  
        }
